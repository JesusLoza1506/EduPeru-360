...
<div class="matricula-header d-flex align-items-center justify-content-between">
    <div>
        <span class="matricula-title"><i class="bi bi-person-badge me-2"></i> Editar Asignación de Curso a Docente</span>
        <p class="mb-0 mt-2">Modifica los datos de la asignación.</p>
    </div>
...
@endif
<form action="{{ route('asignaciones.update', $asignacion->id) }}" method="POST" class="bg-white rounded shadow p-4 mt-4" style="max-width:600px; margin:auto;">
    @csrf
    @method('PUT')
    <div class="mb-3">
        ...
        <select name="curso_id" id="curso_id" class="form-select" required>
            ...
            <option value="{{ $curso->id }}" {{ old('curso_id', $asignacion->curso_id) == $curso->id ? 'selected' : '' }}>{{ $curso->nombre }}</option>
            @endforeach
        </select>
    </div>
    <div class="mb-3">
    ...
    <div class="mb-3">
        ...
        <select name="ano_escolar_id" id="ano_escolar_id" class="form-select" required>
            ...
            <option value="{{ $anio->id }}" {{ old('ano_escolar_id', $asignacion->ano_escolar_id) == $anio->id ? 'selected' : '' }}>{{ $anio->ano }}</option>
            @endforeach
        </select>
    </div>
    <div class="d-flex justify-content-end">
        <button type="submit" class="btn btn-matricula">Actualizar asignación</button>
    </div>
</form>
@endsection